...
class StandaloneMLService:
    ...
    def detect_sentiment(self, query):
        """Detect user sentiment (frustrated, neutral, happy)"""
        frustrated_words = ['angry', 'bad', 'poor', 'worst', 'unacceptable', 'terrible', 'horrible', 'annoying', 'hate', 'disappointed', 'useless', 'fail', 'failure', 'broken', 'wrong', 'help', 'emergency', 'urgent', 'immediately', 'frustrated', 'frustrating', 'slow', 'unhappy']
        positive_words = ['good', 'great', 'excellent', 'thanks', 'thank', 'happy', 'resolved', 'fixed', 'helpful', 'better', 'improved']
        ...
        if frustrated_count > positive_count:
            return 'negative', min(1.0, 0.5 + (frustrated_count * 0.1))
        elif positive_count > frustrated_count:
            return 'positive', min(1.0, 0.5 + (positive_count * 0.1))
        return 'neutral', 0.5

    def classify_user_query(self, query):
        """Advanced classification using multiple techniques with confidence scoring"""
        words = self.extract_words(query)
        query_lower = query.lower()
        
        # Detect sentiment
        sentiment, sentiment_score = self.detect_sentiment(query)
        
        # 1. Rule-based classification for specific cases
        category, priority = self.rule_based_classification(query_lower, words)
        
        # If sentiment is frustrated, boost priority
        if sentiment == 'frustrated' and priority != 'urgent':
            if priority == 'low': priority = 'medium'
            elif priority == 'medium': priority = 'high'
            elif priority == 'high': priority = 'urgent'
        
        if category:
            return category, priority, 0.95  # High confidence for rules
        ...
        predicted_category = max(category_scores, key=category_scores.get) if max_score > 0 else 'Technical'
        ...
        return predicted_category, predicted_priority, round(confidence, 2)
...
def classify_query(current_user_id, current_user_role):
    ...
    try:
        ...
        # Classify the query
        category, priority, confidence = ml_service.classify_user_query(query_text)
        sentiment, sentiment_score = ml_service.detect_sentiment(query_text)
        ...
        return jsonify({
            ...
            'data': {
                ...
                'classification_method': 'Context-Aware Pattern Matching with Sentiment'
            }
        }), 200
