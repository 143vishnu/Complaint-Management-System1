...
def get_complaint_stats(current_user_id, current_user_role):
    ...
    try:
        ...
        if current_user_role == 'admin':
            complaints_for_sentiment = Complaint.query.all()
        else:
            complaints_for_sentiment = Complaint.query.filter_by(user_id=current_user_id).all()
        
        for comp in complaints_for_sentiment:
            sentiment, _ = ml_service.detect_sentiment(comp.description)
