...
import VoiceComplaints from '../components/features/VoiceComplaints';
...
function NewComplaintForm({ user }) {
  const { token } = useAuth();
  const [inputMode, setInputMode] = useState('text'); // 'text', 'voice', 'video'
  ...
  const handleVoiceComplaintSubmit = async (voiceData) => {
    ...
    try {
      const formDataToSend = new FormData();
      formDataToSend.append('title', voiceData.description.substring(0, 100).trim() || 'Voice Complaint');
      formDataToSend.append('description', voiceData.description.trim());

      // Add audio blob if available
      if (voiceData.audioBlob) {
        formDataToSend.append('attachments', voiceData.audioBlob, 'voice-recording.wav');
        console.log('🎤 Attached audio blob:', {
          size: voiceData.audioBlob.size,
          type: voiceData.audioBlob.type,
          duration: voiceData.duration
        });
      }

      console.log('📤 Submitting voice complaint to:', `${import.meta.env.VITE_API_URL}/api/complaints/submit`);
      ...
      if (data.success) {
        console.log('✅ Voice complaint created successfully!', data.data);
        setSuccess('Voice complaint submitted successfully!');
        setFormData({ title: '', description: '' });
        setFiles([]);
        setClassification(null);
        setCreatedComplaintId(data.data.complaint_id);
        setInputMode('video');
        return true;
      } else {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/50 p-8">
        ...
        <div className="space-y-6">
          <VoiceComplaints 
            onVoiceSubmit={handleVoiceComplaintSubmit}
            complaintCategories={['Technical', 'Academic', 'Hostel/Mess', 'Maintenance']}
          />
          
          {createdComplaintId && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-700">
                <strong>Great!</strong> Your voice complaint has been submitted successfully. 
                You can now add video evidence or additional details.
              </p>
            </div>
          )}
        </div>
          ...
          {createdComplaintId ? (
            <>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-700">
                  You can upload multiple videos to your complaint. All videos will be securely stored and accessible to our support team.
                </p>
              </div>
            </>
          ) : (
            <div className="p-6 text-center bg-gray-50 rounded-lg border border-gray-200">
              <p className="text-gray-600 mb-4">
                You need to create a complaint first before uploading videos.
              </p>
              <button
                onClick={() => setInputMode('text')}
                className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition"
              >
                Create Text Complaint First
              </button>
            </div>
          )}
        </div>
                  ...
                  <div className="flex flex-wrap gap-2 ml-4">
                    {message.suggestions.map((suggestion, idx) => (
                      <button
                        ...
                        className="px-4 py-2 bg-blue-50 text-blue-700 text-sm rounded-full border border-blue-100 hover:bg-blue-100 transition-colors animate-fade-in"
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mb-4 items-center">
                  ...
                  {complaint.status === 'pending' || complaint.status === 'in_progress' ? (
                    <span className="flex items-center space-x-2 bg-green-50 text-green-700 px-3 py-1 rounded-full text-xs font-medium border border-green-100">
            ...
            <div className="mt-8">
              ...
              <div className="relative">
                ...
                <div className="flex justify-between relative z-10">
                  ...
                  {/* Step 2: In Progress */}
                  <div className="flex flex-col items-center w-1/3">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full mb-4 ${getStatusStep(complaint.status) >= 2
                  ...
                  {/* Step 3: Completed */}
                  <div className="flex flex-col items-center w-1/3">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full mb-4 ${getStatusStep(complaint.status) >= 3
    ...
    <div className="flex space-x-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          className={`text-2xl ${disabled ? 'cursor-default' : 'cursor-pointer hover:scale-110'} transition-transform ${star <= rating ? 'text-yellow-400' : 'text-gray-300'
            }`}
        >
          {star <= rating ? '⭐' : '☆'}
        </button>
...
function SettingsSection({ user }) {
  ...
  const [smsEnabled, setSmsEnabled] = useState(user?.sms_notifications || false);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/sms/preferences/${user?.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleSMSRegistration = async () => {
    if (!phoneNumber.trim()) {
      setSaveMessage('Please enter a phone number');
      return;
    }
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/sms/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (data.success) {
        setSaveMessage('Phone number registered successfully!');
        setTimeout(() => setSaveMessage(''), 3000);
      } else {
        setSaveMessage(data.message || 'Failed to register phone number');
      }
    } catch (error) {
      setSaveMessage('Error registering phone number. Please try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600 capitalize"
                  />
                </div>
                <div>
              ...
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Profile information cannot be changed here. Please contact support if you need to update your account details.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'sms' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">Phone Number Registration</h3>
                <p className="text-gray-600">Register your phone number to receive SMS notifications about your complaints.</p>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <div>
                    ...
                    <p className="text-xs text-gray-600 mt-1">Receive SMS updates on complaint status changes</p>
                  </div>
                ...
                <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
                  <p className="text-sm text-amber-800">
                    <strong>Notifications:</strong> We'll send SMS updates when:
                  </p>
                  <ul className="text-sm text-amber-700 mt-2 ml-4 list-disc space-y-1">
                    <li>Your complaint status changes</li>
                    <li>An admin adds a response</li>
                    <li>Your complaint is escalated</li>
                    <li>Your complaint is resolved</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-medium text-purple-900">Email Verification</h4>
                      <p className="text-xs text-purple-700 mt-1">Keep your email address verified for security</p>
                    </div>
                ...
                <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-medium text-green-900">Password</h4>
                      <p className="text-xs text-green-700 mt-1">Change your password regularly to keep your account secure</p>
                    </div>
                    <button
                      disabled
              ...
              <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex justify-between">
                    <span>Last login: Today at 4:30 PM</span>
                    <span className="text-green-600">Active</span>
                  </div>
                  <div className="flex justify-between text-xs text-gray-500">
                    <span>Device: Windows (Chrome)</span>
...
function Dashboard() {
  ...
  const navigation = [
    ...
    { name: 'Profile & Settings', icon: CogIcon, href: 'settings', current: activeSection === 'settings' },
  ];
                ...
                {navigation.map((item) => (
                  <li key={item.name}>
                    <button
                      ...
                      className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-colors ${item.current
